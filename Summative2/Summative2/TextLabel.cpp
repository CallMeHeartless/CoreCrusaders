...
std::map<std::string, std::map<GLchar, Character>> TextLabel::sFontMap;
...
TextLabel::TextLabel(std::string newText, std::string newFont, glm::vec2 pos)
{
	...
	program = shaderLoader.CreateProgram("Resources/Shaders/TextVertexShader.vs", "Resources/Shaders/TextFragmentShader.fs");

	glm::mat4 proj = glm::ortho(0.0f, (GLfloat)Utility::SCR_WIDTH, 0.0f, (GLfloat)Utility::SCR_HEIGHT);
	//glm::mat4 proj = glm::ortho(0.0f, (GLfloat)600, 0.0f, (GLfloat)600); // Remove when util is implemented
	...
	auto key = sFontMap.find(newFont);
	if (key == sFontMap.end()) {
		// Create font
		...
		// Add to map
		auto pair = std::make_pair(newFont, Characters);
		sFontMap.insert(pair);
	}
	else {
		Characters = key->second;
	}
